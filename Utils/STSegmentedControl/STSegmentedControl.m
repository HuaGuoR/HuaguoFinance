...
@synthesize normalImageLeft, normalImageMiddle, normalImageRight, selectedImageLeft, selectedImageMiddle, selectedImageRight,intervalBtn,btnHeight;
...
- (id)initWithFrame:(CGRect)frame {
    if((self = [super initWithFrame:CGRectMake(frame.origin.x, frame.origin.y, frame.size.width, STSegmentedControlHeight)])) {
		...
		momentary = NO;
        intervalBtn = 0;
        btnHeight = STSegmentedControlHeight;
    }
    return self;
}
...
- (void)updateUI {
	...
	if([segments count] > 1)
	{
		...
		for(NSObject *object in segments)
		{
            ...
            if (btnHeight != self.frame.size.height){            
                lastY = (self.frame.size.height - btnHeight)/2;            
            }
            
            
            currentSegmentWidth = currentSegmentWidth - intervalBtn;
			CGRect segmentFrame = CGRectMake(round(lastX) + intervalBtn, lastY, currentSegmentWidth, btnHeight);
			lastX += segmentWidth;
			...
			if (intervalBtn > 0){
                if(selectedSegmentIndex == indexOfObject){
                    [button setBackgroundImage:[UIImage imageNamed:@"ico_btn_selected.png"] forState:UIControlStateNormal];
                    
                }else{
                    [button setBackgroundImage:[UIImage imageNamed:@"ico_btn.png"] forState:UIControlStateNormal];
                }
            }else{            
                if(indexOfObject == 0)
                {
...
- (void)segmentTapped:(id)sender {
    ...
    if (intervalBtn > 0){
         ...
         */
        [button setTitleColor:[UIColor colorWithRed: 0.23
                                              green: 0.58
                                               blue: 0.83
