...
//========================================================================
@synthesize imageURL, placeholderImage, delegate,GIF_disable;
...
- (void)setImageURL:(NSURL *)aURL {
	...
	if(!aURL) {
        ...
        [self setGIFImageData:UIImagePNGRepresentation(self.placeholderImage)];
		imageURL = nil;
		return;
	} else {
	...
	NSData* anData = [[EGOImageLoader sharedImageLoader] NSDataForURL:aURL shouldLoadWithObserver:self];
	
	if(anData) {
		//self.image = anImage;
        [self setGIFImageData:anData];
...
- (void)imageLoaderDidLoad:(NSNotification*)notification {
	...
	NSData* anData = [[notification userInfo] objectForKey:@"responseData"];
	//self.image = anImage;        
    [self setGIFImageData:anData];
...
-(void)setGIFImageData:(NSData*) anData {
    ...
    //NSData *gifImageData = UIImagePNGRepresentation(anImage);
    
	if (anData.length < 4) {
		return;
	}
    
	if (GIF_disable == YES || ![FWGIFImageView isGifImage:anData]) {
		self.image = [UIImage imageWithData: anData];
		return;
	}else{	
        [self decodeGIF:anData]; 
...
+ (NSMutableArray*)getGifFrames:(NSData*)gifImageData {
	FWGIFImageView* gifImageView = [[FWGIFImageView alloc] initWithGIFData:gifImageData];
	if (!gifImageView) {
		return nil;
	}
...
- (id)initWithGIFData:(NSData*)gifImageData {
	...
	if (![FWGIFImageView isGifImage:gifImageData]) {
	...
	if (GIF_disable == YES || GIF_frames.count <= 0) {
		UIImage* image = [UIImage imageWithData:gifImageData];
		return [super initWithImage:image];
	}
